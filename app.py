...
def save_sheet_data(df, worksheet_name):
    try:
        ...
        # Atomic overwrite without ws.clear() to avoid data loss on dropped connection
...
<style>
    ...
    .login-header-sub {
        text-align: center; 
        color: #10B981; /* Green accent color from logo */
        font-size: 1.1rem;
        font-weight: 800;
            ...
            with st.form(f"service_report_form_{tech_id}"):
                ...
                for idx, point in enumerate(checklist_items, 1):
                    ...
                    with col_status:
                        status = st.radio(
                            ...
                            horizontal=False, 
                            key=f"check_{selected_job_id}_{selected_category}_{idx}",
                    ...
                    with col_remark:
                        remark = st.text_input(
                            ...
                            key=f"rem_{selected_job_id}_{selected_category}_{idx}",
                            label_visibility="collapsed"
                        )
                ...
                if submit_report:
                    if not rpt_site_location or not rpt_remarks:
                        ...
                        submit_time_str = datetime.now(TIMEZONE).strftime("%Y-%m-%d %I:%M %p")
        ...
        with col_mgr_a:
            ...
            with st.form("new_job_form"):
                ...
                if st.form_submit_button("Task Created"):
                    # Timezone-aware date/time check comparison
                    selected_datetime = datetime.combine(sched_date, sched_time).replace(tzinfo=TIMEZONE)
